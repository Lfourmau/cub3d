...
/*   Updated: 2021/04/14 13:37:55 by lfourmau         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	sprite_infos(big_struct *bs)
{
	...
	bs->ss->inter_x_sprite = (sqrt(pow(bs->rs->deltax, 2) - 1) * (bs->ss->center_sprite_x + bs->ws->player_pos_x) + (bs->ss->center_sprite_y - bs->ws->player_pos_y)) / (2 * sqrt(pow(bs->rs->deltax, 2) - 1));
	bs->ss->inter_y_sprite = sqrt(pow(bs->rs->deltax, 2) - 1) * bs->ss->inter_x_sprite + bs->ws->player_pos_y - sqrt(pow(bs->rs->deltax, 2) - 1) * bs->ws->player_pos_x;
